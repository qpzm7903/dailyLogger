...
## 当前 Milestone：v3.7.0 多维度输出与标签管理

**目标**: 在架构稳定后，推进标签系统后端化和多维度输出增强，提升用户数据管理灵活性。

**版本策略**:

| 版本 | 类型 | 目标 |
|------|------|------|
| v3.7.0 | MINOR | 标签系统后端化 + 多维度输出增强 |
| v4.0.0 | MAJOR | 保留，仅当确实涉及不兼容变更时启用 |

**重构原则**:

- 标签颜色配置从前端硬编码迁移到后端数据库
- 多输出渠道增强：自定义导出模板、输出格式扩展
- 不破坏现有数据结构和 API 契约
- 每个版本都必须保持行为兼容、可测试、可回滚
...
| 版本 | 方向 | 说明 |
|------|------|------|
| v3.7.0 | 多维度输出与标签管理 | 标签颜色后端化、多输出渠道增强 |
