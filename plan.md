...
|----|------|--------|--------|------|------|
| MAINT-070 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | 🔄 进行中 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-069 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-068 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-067 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-066 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-065 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-063 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |

**备注**: 项目状态非常健康，所有核心功能已实现，测试覆盖完善。等待新需求或用户反馈。

---

## v1.100.0（功能增强 + 发布优化）✅ 已完成

**目标**: 修复发布流程问题，实现用户反馈的功能需求

**版本类型**: MINOR（功能增强）

**Issues**:
- #67: Release 界面缺少版本变更内容
- #66: 截图分析支持用户手动补充信息
- #65: 启动后只做截图记录，不做分析

| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| FIX-011 | #67: Release 发布时包含版本变更内容 | 2pts | HIGH | ✅ 完成 | — |
| FEAT-005 | #66: 截图支持用户手动补充信息 | 3pts | MEDIUM | ✅ 完成 | — |
| FEAT-006 | #65: 启动后仅截图记录，延迟分析 | 5pts | MEDIUM | ✅ 完成 | — |

**实现内容**:

**FIX-011 (#67)**: Release 发布时包含版本变更内容
- 自动从 README.md 提取版本历史并添加到 Release Notes

**FEAT-005 (#66)**: 截图支持用户手动补充信息
- 后端: 新增 `user_notes` 字段到 records 表
- 后端: 新增 `update_record_user_notes` Tauri 命令
- 前端: ScreenshotModal 中添加用户备注输入功能

**FEAT-006 (#65)**: 启动后仅截图记录，延迟分析
- 后端: 新增 `capture_only_mode` 设置字段
- 后端: capture_and_store 中支持跳过 AI 分析
- 前端: CaptureSettings 中添加"仅截图模式"开关
- 国际化: 添加中英文翻译

**完成情况**:
- ✅ FIX-011: Release notes 包含版本变更内容
- ✅ FEAT-005: 截图记录支持用户备注
- ✅ FEAT-006: 仅截图模式功能完成
- ✅ Rust 测试通过（435 个）
- ✅ 前端测试通过（939 个）
...
## v1.99.0（按日期重新分析功能）✅ 已完成

**目标**: 实现 Issue #64 - 支持选择任意日期重新分析，以及对单条记录重新分析

**版本类型**: MINOR（功能增强）

**Issue**: #64 - 不仅仅要支持重新分析今天，还要支持可选择某天来分析，以及支持对某记录进行重新分析

**需求分析**:
- ✅ 支持对某记录进行重新分析 - 已实现（v1.43.0 FEAT-001: ScreenshotModal 中的重新分析按钮）
- ✅ 支持选择某天来分析 - 已实现（FEAT-004）

| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| FEAT-004 | 支持选择任意日期重新分析记录 (#64) | 3pts | HIGH | ✅ 完成 | — |

**实现内容**:
1. ✅ 后端: 新增 `reanalyze_records_by_date` 命令，支持传入日期参数
2. ✅ 前端: 创建 ReanalyzeByDateModal 组件，允许用户选择要重新分析的日期
3. ✅ 集成到 Dashboard，添加"按日期重新分析"按钮

**完成情况**:
- ✅ 新增 `reanalyze_records_by_date` Tauri 命令
- ✅ 创建 ReanalyzeByDateModal.vue 组件
- ✅ Dashboard 添加"按日期重新分析"按钮
- ✅ 添加国际化翻译（中英文）
- ✅ 添加组件测试（20 个测试用例）
- ✅ Rust 测试通过（435 个）
- ✅ 前端测试通过（939 个）
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-062 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |

**备注**: 项目状态非常健康，所有核心功能已实现，测试覆盖完善。等待新需求或用户反馈。

---

## v1.94.0（批量重新分析功能）✅ 已完成

**目标**: 实现 Issue #63 - 批量重新分析当天记录功能

**版本类型**: MINOR（功能增强）

**Issue**: #63 - 即使AI工具无法连通，也需要记录，每天生成报的时候，可以再分析一次，或者增加一个功能，把今天的再分析一遍

**需求分析**:
- ✅ 记录与分析分开 - 已实现（FIX-009: AI分析失败时仍保存截图并加入离线队列）
- ✅ 批量重新分析当天记录 - 已实现（FEAT-003）

| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| FEAT-003 | 批量重新分析当天记录 (#63) | 3pts | HIGH | ✅ 完成 | — |

**实现内容**:
1. ✅ 后端: 新增 `reanalyze_today_records` 命令
2. ✅ 前端: 添加"重新分析今天"按钮
3. ✅ 返回成功/失败统计和错误详情

**完成情况**:
- ✅ 新增 `reanalyze_today_records` Tauri 命令
- ✅ Dashboard.vue 添加"重新分析今天"按钮
- ✅ App.vue 添加批量重新分析处理函数
- ✅ Rust 测试通过（435 个）
- ✅ 前端测试通过（919 个）
- ✅ CI 通过
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-057 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-056 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-055 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-054 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-053 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-052 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-051 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-050 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-049 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
**版本类型**: MINOR（Bug 修复 + UX 改进）

**Issues 已修复**:
- ✅ #59: 当模型无法连接时，截图自动保存并加入离线队列
- ✅ #60: 历史记录展示内容改为多行显示（移除 truncate）
- ✅ #61: 点击待同步徽章显示待处理任务详情
- ✅ #62: 提高深色主题文字对比度（gray-400 → gray-300）

| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| FIX-009 | #59: 模型无法连接时截图自动保存 | 2pts | HIGH | ✅ 完成 | — |
| FIX-010 | #60: 历史记录展示内容不全 | 2pts | HIGH | ✅ 完成 | — |
| UX-028 | #61: 显示待同步项目详情 | 2pts | MEDIUM | ✅ 完成 | — |
| UX-029 | #62: 改进深色主题文字可见性 | 2pts | MEDIUM | ✅ 完成 | — |

**完成情况**:
- ✅ HistoryViewer: 使用 line-clamp-3 替代 truncate，显示多行内容
- ✅ HistoryViewer: 标签和时间文字对比度提升（gray-400 → gray-300）
- ✅ Header: 时间显示对比度提升（gray-400 → gray-300）
- ✅ 新增 OfflineQueueModal 组件显示待同步任务
- ✅ 新增 Tauri 命令 get_pending_offline_tasks
- ✅ 点击待同步徽章打开任务详情模态框
- ✅ capture_and_store: AI 分析失败时仍保存截图并加入重试队列
- ✅ Rust 测试通过（435 个）
- ✅ 前端测试通过（919 个）
- ✅ Clippy 无警告
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-048 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
...
| ID | 需求 | 故事点 | 优先级 | 状态 | Spec |
|----|------|--------|--------|------|------|
| MAINT-046 | 定期健康检查（依赖/测试/CI） | 1pt | LOW | ✅ 完成 | — |
