<template>
  <BaseModal backdrop="medium" :focusTrap="false" contentClass="w-full max-w-lg mx-4" @close="handleClose">
    <!-- Header -->
    <div class="flex items-center justify-between px-6 py-4 border-b border-[var(--color-border)]">
      ...
      <button
        v-if="isCompleted"
        @click="handleClose"
        class="text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors p-1 rounded hover:bg-[var(--color-action-neutral)]"
      >
    ...
    <!-- Step Indicator -->
    <div v-if="!isCompleted" class="px-6 py-4 border-b border-[var(--color-border)]">
      <div class="flex items-center justify-center gap-2">
        <div v-for="step in 3" :key="step" class="flex items-center">
          <div
            class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors"
            :class="step < currentStep ? 'bg-green-500 text-white' : step === currentStep ? 'bg-primary text-white' : 'bg-[var(--color-surface-2)] text-[var(--color-text-secondary)]'"
          >
            <span v-if="step < currentStep">✓</span>
            <span v-else>{{ step }}</span>
          </div>
          <div v-if="step < 3" class="w-12 h-0.5" :class="step < currentStep ? 'bg-green-500' : 'bg-[var(--color-surface-2)]'"></div>
        </div>
      </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input
              ...
              class="w-full bg-[var(--color-surface-0)] border border-[var(--color-border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:border-primary focus:outline-none"
            />
          </div>
          <div>
