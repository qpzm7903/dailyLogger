...
pub async fn generate_daily_summary() -> Result<String, String> {
    ...
    let api_base_url = settings.api_base_url
        .clone()
    ...
    let records_text: String = records.iter().map(|r| {
        let time = chrono::DateTime::parse_from_rfc3339(&r.timestamp)
            .map(|dt| dt.format("%H:%M").to_string())
            .unwrap_or_else(|_| "unknown".to_string());
        
        let source = if r.source_type == "auto" { "🖥️ 自动感知" } else { "⚡ 闪念" };
        
        format!("- [{}] {}: {}", time, source, r.content)
    }).collect::<Vec<_>>().join("\n");
    
    let prompt = format!(r#"你是一个工作日志助手。请根据以下今日工作记录，生成一份结构化的 Markdown 格式日报。
