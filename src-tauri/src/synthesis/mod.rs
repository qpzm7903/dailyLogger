...
mod tests {
    ...
    // ── Performance benchmark tests (CORE-008 Task 3.1) ──

    #[test]
    fn test_benchmark_format_records_100_entries() {
        let records: Vec<Record> = (0..100)
            .map(|i| Record {
                id: i,
                timestamp: format!(
                    "2026-03-15T{:02}:{:02}:00+00:00",
                    (i % 24) as u32,
                    (i % 60) as u32
                ),
                source_type: if i % 3 == 0 { "manual" } else { "auto" }.to_string(),
                content: format!("Record content for entry {} — working on task XYZ", i),
                screenshot_path: None,
                monitor_info: None,
                tags: None,
            })
        ...
        let start = std::time::Instant::now();
        let result = format_records_for_summary(&records);
        let elapsed = start.elapsed();

        assert!(!result.is_empty(), "Formatted output should not be empty");
        assert!(
            result.contains("Record content for entry 0"),
            "Should contain first record"
        );
        assert!(
            result.contains("Record content for entry 99"),
            "Should contain last record"
        );
        // AC #3: Daily summary generation < 30 seconds for 100 records
        // format_records_for_summary is the CPU-bound part (API call is I/O)
        assert!(
            elapsed.as_secs() < 30,
            "Formatting 100 records should complete in < 30 seconds, took: {:?}",
            elapsed
        );
    }

    #[test]
    fn test_benchmark_filter_records_100_entries() {
        let records: Vec<Record> = (0..100)
            .map(|i| Record {
                id: i,
                timestamp: format!(
                ...
                source_type: if i % 2 == 0 { "auto" } else { "manual" }.to_string(),
                content: format!("Filter benchmark record {}", i),
                screenshot_path: None,
                monitor_info: None,
                tags: None,
            })
        ...
        // 50 auto records should remain (even indices)
        assert_eq!(filtered.len(), 50);
        // Should complete almost instantly
        assert!(
            elapsed.as_millis() < 1000,
            "Filtering 100 records should be < 1 second, took: {:?}",
            elapsed
        );
    }

    #[test]
    fn test_benchmark_generate_summary_filename() {
        let settings = Settings {
            summary_title_format: Some("工作日报 - {date}".to_string()),
            ..create_settings_with_include_manual(true)
        };
        ...
        assert!(
            ...
            "1000 filename generations should be < 5 seconds, took: {:?}",
            elapsed
        );
    }
}
