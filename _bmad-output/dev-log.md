...
1. **模式复用**: `work_time.rs` 架构设计完全复用 SMART-002 的 `silent_tracker.rs` 模式。理由：Epic 内保持一致性，加速开发。

2. **14天滑动窗口**: 比 SMART-002 的 7 天更长，理由：工作时间模式需要更多数据才能准确识别，14 天覆盖两周工作周期。

3. **跨午夜时间段支持**: 使用 `start <= end` 判断正常时段，`start > end` 判断跨午夜时段。理由：支持夜班用户场景（如 22:00-06:00）。

4. **优先级设计**: 自定义工作时间 > 学习到的工作时间 > 默认允许捕获。理由：用户明确设置优先级最高。

### 问题与解决

**Settings 字段同步更新**: 新增 5 个字段需同步更新 5 处代码。解决：建立新增字段检查清单。

### 后续约定

- **学习模块命名**: `*_learner.rs` 或 `*_tracker.rs` 后缀，使用 `Lazy<Mutex<...>>` 全局单例
- **跨午夜时间判断**: `if start <= end { normal_check } else { cross_midnight_check }`
- **学习进度显示**: 前端通过 `get_*_status` 命令获取实时状态，增强用户信任

### 技术债务追踪

| 项目 | 来源 | 优先级 | 状态 |
|-----|------|--------|------|
| 托盘图标休眠状态 | SMART-003 Task 4 子任务 | Low | 待处理 |
| 学习数据持久化 | SMART-003 Task 2 可选 | Low | 待评估 |
