...
### 洞察 4：导出功能复用性高
DATA-004 建立的导出基础设施（日期范围选择、格式化模板、进度反馈模式）可以直接复用到后续报告输出与本地文件写出能力。
...
### 当前 Backlog 状态
后续工作应继续围绕核心记录体验展开，优先处理测试、性能与数据链路质量问题。

### 准备建议
1. **复用 DATA-004 导出基础设施**：后续报告输出或本地导出能力可以继续基于现有的 `export/mod.rs` 模式扩展
2. **优先处理前端测试债务**：在开始新 Epic 前补充 Tag* 组件的基础测试
3. **考虑性能基线**：当前 286 个测试的运行时间应作为基线，避免后续测试集膨胀导致 CI 变慢
4. **避免非核心外部耦合**：新增能力优先采用本地优先方案，确需外部依赖时再单独评估成本与维护负担
