...
- [ ] Task 1: 数据库统计查询函数 (AC: #2, #3)
  - [ ] 1.1 在 `memory_storage/mod.rs` 添加 `get_statistics()` 函数
  - [ ] 1.2 实现按日期范围查询统计数据逻辑
  - [ ] 1.3 添加 Rust 单元测试

- [ ] Task 2: Tauri 命令注册 (AC: #1)
  - [ ] 2.1 在 `main.rs` 注册 `get_statistics` 命令
  - [ ] 2.2 定义统计结果的数据结构 `Statistics`

- [ ] Task 3: 前端统计面板 UI (AC: #1, #2, #3, #4)
  - [ ] 3.1 创建 `StatisticsPanel.vue` 组件
  - [ ] 3.2 实现时间范围选择器（今日/本周/本月/自定义）
  - [ ] 3.3 实现统计卡片展示（截图数、时段数、分析成功率）
  - [ ] 3.4 实现简单柱状图展示每日趋势（可复用 ScreenshotGallery 的展示模式）

- [ ] Task 4: 数据导出功能 (AC: #5)
  - [ ] 4.1 在前端添加导出按钮
  - [ ] 4.2 实现 CSV/Markdown 格式导出逻辑
