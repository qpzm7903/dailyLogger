...
- [x] Task 1: 数据库查询函数 (AC: #1, #2)
  - [x] 1.1 在 `memory_storage/records.rs` 添加 `get_records_by_session_id(session_id: i64)`
  - [x] 1.2 在 `session_manager/mod.rs` 添加 `get_previous_session_context(session_id: i64)`
  - [x] 1.3 添加 `SessionScreenshot` 结构体

- [x] Task 2: 批量分析 Prompt 设计 (AC: #3)
  - [x] 2.1 设计 multi-image prompt 结构
  - [x] 2.2 定义 API 返回 JSON Schema（`SessionAnalysisResponse`）
  - [x] 2.3 编写 DEFAULT_SESSION_ANALYSIS_PROMPT 常量

- [x] Task 3: 实现 analyze_session 核心函数 (AC: #3, #4)
  - [x] 3.1 实现 `collect_session_screenshots()` - 收集时段截图
  - [x] 3.2 实现 `build_multi_image_request()` - 构建多图 API 请求
  - [x] 3.3 实现 `call_vision_api_batch()` - 批量 Vision API 调用
  - [x] 3.4 实现 `parse_and_store_results()` - 解析并存储结果
  - [x] 3.5 组装 `analyze_session()` 主函数

- [x] Task 4: Tauri Commands 暴露 (AC: #5)
  - [x] 4.1 添加 `#[command] async fn analyze_session(session_id: i64)`
  - [x] 4.2 添加 `#[command] async fn get_session_screenshots(session_id: i64)`
  - [x] 4.3 在 `main.rs` 的 `generate_handler![]` 中注册新命令

- [x] Task 5: 错误处理与边界情况 (AC: #6)
  - [x] 5.1 处理截图文件不存在的情况
  - [x] 5.2 处理 API 调用失败的情况
  - [x] 5.3 处理空时段（无截图）的情况
  - [x] 5.4 处理 JSON 解析失败的情况

- [x] Task 6: 测试验证 (AC: #7)
  - [x] 6.1 编写 `analyze_session` 单元测试
  - [x] 6.2 运行 `cargo fmt`
...
1. **数据库查询函数**：
   - 在 `records.rs` 添加了 `SessionScreenshot` 结构体
   - 添加了 `get_records_by_session_id()` 查询指定时段的待分析截图
   - 添加了 `update_record_analysis()` 更新记录分析结果
   - 添加了 `update_session_analysis()` 更新时段摘要和上下文

2. **session_manager 模块扩展**：
   - 添加了 `get_previous_session_context()` 获取上一时段上下文
   - 添加了 `ScreenshotAnalysis` 和 `SessionAnalysisResponse` 结构体
   - 添加了 `DEFAULT_SESSION_ANALYSIS_PROMPT` 常量
   - 添加了 `ApiConfig` 结构体和 `load_api_config()` 函数
   - 添加了 `encode_screenshot()` 读取并编码截图为 base64
   - 添加了 `build_multi_image_request()` 构建多图 API 请求
   - 添加了 `call_vision_api_batch()` 批量 Vision API 调用
   - 添加了 `analyze_session()` 主函数（Tauri command）
   - 添加了 `get_session_screenshots()` Tauri command

3. **Tauri 命令注册**：
   - 在 `main.rs` 的 `generate_handler![]` 中注册了 `get_today_sessions`、`analyze_session`、`get_session_screenshots`
