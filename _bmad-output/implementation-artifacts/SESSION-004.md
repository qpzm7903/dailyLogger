...
- [ ] Task 1: 前端时段列表组件 (AC: #1, #2)
  - [ ] 1.1 创建或扩展 SessionList 组件显示时段列表
  - [ ] 1.2 添加状态筛选（pending/analyzed/all）
  - [ ] 1.3 显示每个时段的时长和截图数量

- [ ] Task 2: 手动分析 UI 交互 (AC: #2, #3)
  - [ ] 2.1 为待分析时段添加"分析"按钮
  - [ ] 2.2 调用 `invoke('analyze_session', {session_id})`
  - [ ] 2.3 添加 loading 状态和 toast 通知
...
- [ ] Task 4: 错误处理 (AC: #5)
  - [ ] 4.1 捕获 analyze_session 错误
  - [ ] 4.2 显示 toast 错误消息
  - [ ] 4.3 提供重试选项

- [ ] Task 5: 测试验证 (AC: #6)
  - [ ] 5.1 运行 `cargo fmt`
  - [ ] 5.2 运行 `cargo clippy -- -D warnings`
  - [ ] 5.3 运行 `cargo test --no-default-features`
