...
- [x] Task 1: 实现工作时间模式学习 (AC: 1)
  - [x] 创建 `WorkTimePatternLearner` 结构体
  - [x] 记录每次捕获的时间戳，按小时统计活动分布
  - [x] 实现滑动窗口统计（最近 14 天）
  - [x] 设计学习算法：识别高频活动时段
  - [x] 编写学习算法单元测试

- [x] Task 2: 扩展数据库 Schema (AC: 1, 2, 3)
  - [x] 在 settings 表添加 `auto_detect_work_time` 字段（布尔值，默认开启）
  - [x] 在 settings 表添加 `use_custom_work_time` 字段（布尔值）
  - [x] 在 settings 表添加 `custom_work_time_start` 字段（时间字符串，如 "09:00"）
  - [x] 在 settings 表添加 `custom_work_time_end` 字段（时间字符串，如 "18:00"）
  - [x] 在 settings 表添加 `learned_work_time` 字段（JSON，存储学习结果）
  - [ ] 创建 `work_time_history` 表存储每日活动分布（可选）
  - [x] 更新 `Settings` 结构体添加新字段
  - [x] 编写数据库迁移测试

- [x] Task 3: 实现工作时间判断逻辑 (AC: 2)
  - [x] 创建 `is_work_time()` 函数
  - [x] 实现时间段匹配逻辑（支持跨午夜）
  - [x] 实现学习结果解析
  - [x] 处理边界情况（午休、加班）
  - [x] 编写边界测试
...
- [x] Task 4: 集成到捕获流程 (AC: 1, 2)
  - [x] 修改 `start_auto_capture()` 检查工作时间
  - [x] 在捕获循环中检查当前时间是否在工作时间内
  - [x] 实现自动暂停/恢复逻辑
  - [ ] 更新托盘图标状态显示休眠/活跃
  - [x] 记录工作时间变化日志
