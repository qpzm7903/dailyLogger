...
- [x] Task 1: 数据库扩展 - 自定义报告配置 (AC: #1)
  - [x] 1.1 在 Settings 表添加 `custom_report_prompt` 和 `last_custom_report_path` 字段
  - [x] 1.2 预设模板通过代码计算（双周报=14天、季度报=当前季度），无需额外 DB 字段
  - [x] 1.3 更新 Settings struct 和 CRUD 函数（get_settings_sync、save_settings_sync）
  - [x] 1.4 添加数据库迁移逻辑（ALTER TABLE ADD COLUMN）

- [x] Task 2: Rust 后端 - 自定义周期报告生成 (AC: #2, #3, #4, #6)
  - [x] 2.1 复用已有的 `get_records_by_date_range_sync()` 函数（已存在于 memory_storage）
  - [x] 2.2 在 synthesis/mod.rs 添加 `generate_custom_report()` Tauri command
    - 参数: start_date, end_date, report_name (可选)
    - 复用 `format_records_for_summary()` 和 `format_records_by_week()` 格式化记录
    - 复用 LLM 调用模式
    - 生成文件名: `{report_name}-{start_date}-to-{end-date}.md`
  - [x] 2.3 添加日期范围计算辅助函数:
    - `get_biweekly_range()` - 计算双周范围（最近14天）
    - `get_quarter_range()` - 计算当前季度范围
    - `generate_custom_report_filename()` - 生成文件名
    - `get_default_custom_report_prompt()` - 默认 prompt 模板
  - [x] 2.4 在 main.rs 的 `generate_handler![]` 中注册 `generate_custom_report`
  - [x] 2.5 编写单元测试（12个测试）:
    - custom_report_prompt 内容和非空测试
    - filename 生成测试（默认名、自定义名、季度名）
    - biweekly_range 14天跨度和今日结束测试
    - quarter_range Q1/Q4/有效日期/首日验证测试

- [x] Task 3: 前端 - 自定义周期报告 UI (AC: #1, #2, #3, #4, #5)
  - [x] 3.1 创建 `CustomReportModal.vue` 组件
    - 日期范围选择器 (两个 date input)
    - 预设选项: 双周报、季度报、自定义
    - 报告名称输入 (可选)
  - [x] 3.2 在 App.vue 添加"自定义报告"入口按钮（橙色主题）
  - [x] 3.3 实现日期选择和预设切换逻辑
  - [x] 3.4 添加报告生成 loading 状态和成功/错误提示
  - [x] 3.5 显示最近报告路径和打开按钮（DailySummaryViewer 复用）

- [x] Task 4: 测试 (AC: All)
  - [x] 4.1 前端 Vitest 测试: 12个 CustomReportModal 组件测试
  - [x] 4.2 Rust 单元测试: 12个 synthesis 模块测试
...
- Settings struct 扩展 2 个字段: `custom_report_prompt`, `last_custom_report_path`
- 数据库迁移使用幂等 ALTER TABLE ADD COLUMN
- 复用已有 `get_records_by_date_range_sync()` 查询函数
- `generate_custom_report()` 命令支持 start_date, end_date, report_name 参数
- 超过 14 天的报告使用 `format_records_by_week()` 分组格式
- 前端 CustomReportModal 支持 3 种预设 + 自定义日期范围
- 所有 test DB schema 同步更新（memory_storage、manual_entry、auto_perception）
- Rust 267 tests passed (新增 12), Frontend 191 tests passed (新增 12)

### File List

- `src-tauri/src/memory_storage/mod.rs` - Settings struct 新增字段 + DB 迁移 + CRUD 更新 + test schema 更新
- `src-tauri/src/synthesis/mod.rs` - 新增 custom report prompt/filename/range 函数 + generate_custom_report command + 12 个单元测试
- `src-tauri/src/main.rs` - 注册 generate_custom_report 命令
- `src-tauri/src/manual_entry/mod.rs` - test schema 更新
- `src-tauri/src/auto_perception/mod.rs` - test schema 更新
- `src/components/CustomReportModal.vue` - 新建自定义报告模态框
