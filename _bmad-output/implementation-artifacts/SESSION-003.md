...
- [ ] Task 1: 数据库查询函数 (AC: #1, #2, #5)
  - [ ] 1.1 在 `memory_storage/records.rs` 添加 `update_record_user_notes(record_id, user_notes)` 函数
  - [ ] 1.2 在 `session_manager/mod.rs` 添加 `update_session_user_summary(session_id, user_summary)` 函数
  - [ ] 1.3 添加 `get_record_with_session(record_id)` 返回带 session 信息的记录

- [ ] Task 2: Tauri Commands 暴露 (AC: #5)
  - [ ] 2.1 添加 `#[command] async fn update_record_user_notes(record_id: i64, user_notes: String)`
  - [ ] 2.2 添加 `#[command] async fn update_session_user_summary(session_id: i64, user_summary: String)`
  - [ ] 2.3 在 `main.rs` 的 `generate_handler![]` 中注册新命令

- [ ] Task 3: 前端截图编辑 UI (AC: #3)
  - [ ] 3.1 在 ScreenshotModal.vue 中添加"编辑备注"按钮
  - [ ] 3.2 添加 textarea 用于编辑 user_notes
  - [ ] 3.3 实现保存/取消编辑逻辑
  - [ ] 3.4 调用 `update_record_user_notes` Tauri command
  - [ ] 3.5 编辑后更新 modal 中的展示内容
...
- [ ] Task 5: 日报生成适配 (AC: #4)
  - [ ] 5.1 修改 `synthesis/mod.rs` 中的记录获取逻辑
  - [ ] 5.2 使用 `COALESCE(user_notes, content)` 优先展示用户内容
  - [ ] 5.3 使用 `COALESCE(user_summary, ai_summary)` 优先展示用户摘要

- [ ] Task 6: 测试验证 (AC: #7)
  - [ ] 6.1 编写 `update_record_user_notes` 单元测试
  - [ ] 6.2 编写 `update_session_user_summary` 单元测试
  - [ ] 6.3 运行 `cargo fmt`
  - [ ] 6.4 运行 `cargo clippy -- -D warnings`
  - [ ] 6.5 运行 `cargo test --no-default-features`
